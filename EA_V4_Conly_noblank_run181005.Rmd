...
One line of the sequence was entered without an EA start. This created several files with the wrong sample data in them, and is why there is so much renaming.  
samples culled or too small for linearity range: 



15191 was removied from the linarity for the purpose of calculating the k factor for % c. 

Standards culled and why:

15188 and 15241 could be removied?
...
```{r correcting for identifier offset or renaming stuff}

for (ID in c(15357:15354)){
  ID2 <- ID-1
...
data[ data$Analysis == 15353 , "Identifier1"] <- "17 20ZR 13C sup"
data[ data$Analysis == 15353 , "mass"] <- "1451"
data[ data$Analysis == 15353 , "carousel"] <- "5"
data[ data$Analysis == 15353 , "type"] <- "sample"

data[ data$Analysis == 15359 , "Identifier1"] <- "26 20ZR 13C"

data[ data$Analysis == 15352 , "type"] <- "sample"
...
stds.to.cull <- c(NA) #Analysis with yield problem and/or significant outlier in d13C or d18O, can rerun this line after 1st round to see outiers, than add them here   
...
```{r plot of bad std}
#HOS_15241<- iso_plot_continuous_flow_data(dxf_files$`15241__HOS_403.dxf`, c(44,45,46))
#ggplotly(HOS_15241)
```
```{r}
#iso_plot_continuous_flow_data(dxf_files$`13961__WF2-35_12003.dxf`  , c(44,45,46))
```
